...
# ⚖️ Reconciliation Agent

> Concilia ledger, exchange y blockchain mediante claves estructuradas y explica cada diferencia sin corregir saldos.
...
Producir una conciliación reproducible entre ledger, exchange y blockchain, con excepciones trazables y revisión humana.
...
Úsalo para demostrar si los registros contables, del exchange y on-chain representan las mismas operaciones.
...
### 1 · Caso financiero — Conciliación normal

**El caso.** El cierre diario contiene 1.248 asientos, 612 operaciones del exchange y 609 transacciones on-chain confirmadas. Los identificadores están completos y el período abarca veinticuatro horas de operación autorizada.

**Le escribes:**

```text
Concilia el cierre diario entre ledger, exchange y blockchain y entrega las excepciones.
```
...
- **Resultado estructurado** — `reconciliation_report` con referencias de entrada y evidencia.
...
### 2 · Caso financiero — Conciliación con excepción

**El caso.** Un retiro aparece confirmado on-chain y debitado por el exchange, pero no existe en el ledger interno. La discrepancia debe conservar su rastro hasta el registro de origen sin corregirlo.

**Le escribes:**

```text
Localiza la transacción faltante y explica exactamente qué fuentes no coinciden.
```
...
### 3 · Caso financiero — Conciliación con falso positivo

**El caso.** Una transferencia interna se registró dos veces por usar hash y referencia de lote como claves distintas. El analista necesita saber por qué saltó la regla y qué evidencia permite cerrarla.

**Le escribes:**

```text
Revisa este posible duplicado y determina si es un doble registro o dos operaciones reales.
```
...
flowchart LR
    ...
    AG(["⚖️ reconciliation-agent"])
    OUT["📦 Te entrega<br/>· reconciliation_report<br/>· evidence_refs<br/>· confidence_assessment<br/>· human_review_packet<br/>· audit_events"]
...
| Propiedad | Valor |
|---|---|
| Identificador | `reconciliation-agent` |
...
```bash
operational-agents inspect reconciliation-agent
operational-agents export claude --target ~/.claude/agents
claude --agent reconciliation-agent
```
