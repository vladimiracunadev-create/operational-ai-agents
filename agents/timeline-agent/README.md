...
> Construye líneas temporales normalizadas y trazables desde eventos heterogéneos sin inventar orden causal.
...
Entregar una cronología reproducible que separe tiempo observado, normalizado, estimado y desconocido.
...
Úsalo para ordenar un incidente financiero entre sistemas, zonas horarias y relojes potencialmente desalineados.
...
### 1 · Caso financiero — Línea temporal normal

**El caso.** Los eventos provienen de ledger, exchange, nodo, IAM y tickets, usando UTC, hora local y precisión desigual. Los identificadores están completos y el período abarca veinticuatro horas de operación autorizada.

**Le escribes:**

```text
Construye una línea temporal única de este caso y conserva la fuente de cada marca de tiempo.
```
...
- **Resultado estructurado** — `normalized_timeline` con referencias de entrada y evidencia.
...
### 2 · Caso financiero — Línea temporal con excepción

**El caso.** Un evento IAM parece posterior al retiro porque un sistema registró hora local sin zona y otro usó UTC. La discrepancia debe conservar su rastro hasta el registro de origen sin corregirlo.

**Le escribes:**

```text
Normaliza estas marcas de tiempo y determina qué orden puede sostener la evidencia.
```
...
### 3 · Caso financiero — Línea temporal con falso positivo

**El caso.** Dos eventos comparten el mismo segundo, pero una interfaz redondeó milisegundos y sugiere causalidad inexistente. El analista necesita saber por qué saltó la regla y qué evidencia permite cerrarla.

**Le escribes:**

```text
Revisa esta secuencia aparentemente causal y expresa la incertidumbre temporal correctamente.
```
...
flowchart LR
    ...
    AG(["🕒 timeline-agent"])
    OUT["📦 Te entrega<br/>· normalized_timeline<br/>· evidence_refs<br/>· confidence_assessment<br/>· human_review_packet<br/>· audit_events"]
...
```bash
operational-agents inspect timeline-agent
operational-agents export claude --target ~/.claude/agents
claude --agent timeline-agent
```
