...
> Mapea evidencia a controles y períodos de auditoría, identifica lagunas y evita afirmar cumplimiento sin respaldo.
...
Producir una matriz control-evidencia reproducible con cobertura, vigencia, propietario y excepciones explícitas.
...
Úsalo para preparar evidencia de controles financieros para auditoría o revisión de cumplimiento.
...
### 1 · Caso financiero — Evidencia de cumplimiento normal

**El caso.** La revisión trimestral cubre 18 controles, 54 artefactos y responsables distintos para IAM, wallets y conciliación. Los identificadores están completos y el período abarca veinticuatro horas de operación autorizada.

**Le escribes:**

```text
Mapea esta evidencia a los controles del período y señala cobertura, vigencia y lagunas.
```
...
- **Resultado estructurado** — `control_evidence_matrix` con referencias de entrada y evidencia.
...
### 2 · Caso financiero — Evidencia de cumplimiento con excepción

**El caso.** Un control exige revisión mensual, pero solo existen dos aprobaciones para un trimestre de tres meses. La discrepancia debe conservar su rastro hasta el registro de origen sin corregirlo.

**Le escribes:**

```text
Documenta esta evidencia faltante sin convertir ausencia de prueba en prueba de incumplimiento.
```
...
### 3 · Caso financiero — Evidencia de cumplimiento con falso positivo

**El caso.** El mismo reporte respalda dos controles relacionados y fue marcado erróneamente como reutilización inválida. El analista necesita saber por qué saltó la regla y qué evidencia permite cerrarla.

**Le escribes:**

```text
Comprueba si esta evidencia puede soportar ambos controles y deja claro su alcance exacto.
```
...
flowchart LR
    ...
    AG(["📋 compliance-evidence-agent"])
    OUT["📦 Te entrega<br/>· control_evidence_matrix<br/>· evidence_refs<br/>· confidence_assessment<br/>· human_review_packet<br/>· audit_events"]
...
| Propiedad | Valor |
|---|---|
| Identificador | `compliance-evidence-agent` |
...
```bash
operational-agents inspect compliance-evidence-agent
operational-agents export claude --target ~/.claude/agents
claude --agent compliance-evidence-agent
```
