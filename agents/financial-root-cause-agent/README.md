...
# 🔬 Financial Root Cause Agent

> Investiga causas raíz de incidentes financieros mediante hipótesis contrastadas sin ejecutar remediaciones ni acciones sobre activos.
...
Construir una explicación causal reproducible desde cronología y evidencia, reservando toda remediación a revisión humana.
...
Úsalo para explicar por qué ocurrió un error contable u operación no autorizada sin convertir correlación en causalidad.
...
### 1 · Caso financiero — Causa raíz financiera normal

**El caso.** El caso reúne una cronología validada, hallazgos de conciliación e IAM y evidencia con procedencia comprobada. Los identificadores están completos y el período abarca veinticuatro horas de operación autorizada.

**Le escribes:**

```text
Construye el análisis de causa raíz de este incidente financiero usando solo la evidencia disponible.
```
...
- **Resultado estructurado** — `root_cause_report` con referencias de entrada y evidencia.
...
### 2 · Caso financiero — Causa raíz financiera con excepción

**El caso.** Un retiro no autorizado coincide con una elevación temporal de permisos y una excepción de conciliación posterior. La discrepancia debe conservar su rastro hasta el registro de origen sin corregirlo.

**Le escribes:**

```text
Contrasta hipótesis para esta operación no autorizada sin atribuir intención ni ejecutar remediaciones.
```
...
### 3 · Caso financiero — Causa raíz financiera con falso positivo

**El caso.** Un cambio IAM ocurrió cerca del incidente, pero los registros muestran que la identidad no participó en la operación. El analista necesita saber por qué saltó la regla y qué evidencia permite cerrarla.

**Le escribes:**

```text
Revisa esta correlación temporal y descarta causas que no estén respaldadas por un mecanismo verificable.
```
...
flowchart LR
    ...
    AG(["🔬 financial-root-cause-agent"])
    OUT["📦 Te entrega<br/>· root_cause_report<br/>· evidence_refs<br/>· confidence_assessment<br/>· human_review_packet<br/>· audit_events"]
...
| Propiedad | Valor |
|---|---|
| Identificador | `financial-root-cause-agent` |
...
```bash
operational-agents inspect financial-root-cause-agent
operational-agents export claude --target ~/.claude/agents
claude --agent financial-root-cause-agent
```
