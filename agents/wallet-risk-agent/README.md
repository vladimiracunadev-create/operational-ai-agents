...
> Analiza direcciones nuevas, retiros grandes, velocidad y conducta inusual sin controlar wallets ni firmar transacciones.
...
Detectar patrones de riesgo de wallet con reglas reproducibles, contexto histórico y evidencia on-chain referenciada.
...
Úsalo para priorizar actividad de wallets que requiere investigación humana basada en señales explicables.
...
### 1 · Caso financiero — Riesgo de wallet normal

**El caso.** La muestra contiene seis meses de retiros, perfiles históricos por wallet y etiquetas autorizadas de contraparte. Los identificadores están completos y el período abarca veinticuatro horas de operación autorizada.

**Le escribes:**

```text
Evalúa direcciones nuevas, importes, velocidad y comportamiento inusual de estas wallets.
```
...
- **Resultado estructurado** — `wallet_risk_findings` con referencias de entrada y evidencia.
...
### 2 · Caso financiero — Riesgo de wallet con excepción

**El caso.** Una dirección nueva recibe cinco retiros grandes en doce minutos y no tiene historial de contraparte. La discrepancia debe conservar su rastro hasta el registro de origen sin corregirlo.

**Le escribes:**

```text
Analiza esta secuencia de retiros y prioriza el riesgo sin bloquear ni mover fondos.
```
...
### 3 · Caso financiero — Riesgo de wallet con falso positivo

**El caso.** Una wallet de tesorería rota por política dispara la regla de dirección nueva durante una migración aprobada. El analista necesita saber por qué saltó la regla y qué evidencia permite cerrarla.

**Le escribes:**

```text
Revisa esta alerta de dirección nueva considerando la rotación de tesorería documentada.
```
...
flowchart LR
    ...
    AG(["👛 wallet-risk-agent"])
    OUT["📦 Te entrega<br/>· wallet_risk_findings<br/>· evidence_refs<br/>· confidence_assessment<br/>· human_review_packet<br/>· audit_events"]
...
| Propiedad | Valor |
|---|---|
| Identificador | `wallet-risk-agent` |
...
```bash
operational-agents inspect wallet-risk-agent
operational-agents export claude --target ~/.claude/agents
claude --agent wallet-risk-agent
```
