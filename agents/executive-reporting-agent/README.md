...
> Sintetiza hallazgos financieros para decisión ejecutiva conservando severidad, confianza, evidencia y límites.
...
Entregar un resumen ejecutivo accionable y trazable que reserve toda decisión financiera u operacional a personas autorizadas.
...
Úsalo para convertir hallazgos técnicos revisados en un informe ejecutivo sin ocultar incertidumbre ni inventar impacto.
...
### 1 · Caso financiero — Reporte ejecutivo normal

**El caso.** El comité recibirá hallazgos de conciliación, IAM, wallets y trading con escalas y niveles de confianza diferentes. Los identificadores están completos y el período abarca veinticuatro horas de operación autorizada.

**Le escribes:**

```text
Prepara un reporte ejecutivo trazable de estos hallazgos para el comité de riesgo.
```
...
- **Resultado estructurado** — `executive_risk_brief` con referencias de entrada y evidencia.
...
### 2 · Caso financiero — Reporte ejecutivo con excepción

**El caso.** Existe un hallazgo crítico con evidencia parcial y alto impacto potencial, pero aún sin pérdida confirmada. La discrepancia debe conservar su rastro hasta el registro de origen sin corregirlo.

**Le escribes:**

```text
Resume este hallazgo crítico sin presentar impacto potencial como pérdida observada.
```
...
### 3 · Caso financiero — Reporte ejecutivo con falso positivo

**El caso.** Tres alertas técnicas se refieren al mismo evento y contarlas por separado exageraría el riesgo agregado. El analista necesita saber por qué saltó la regla y qué evidencia permite cerrarla.

**Le escribes:**

```text
Consolida estas alertas correlacionadas y evita duplicar su impacto en el reporte.
```
...
flowchart LR
    ...
    AG(["📊 executive-reporting-agent"])
    OUT["📦 Te entrega<br/>· executive_risk_brief<br/>· evidence_refs<br/>· confidence_assessment<br/>· human_review_packet<br/>· audit_events"]
...
| Propiedad | Valor |
|---|---|
| Identificador | `executive-reporting-agent` |
...
```bash
operational-agents inspect executive-reporting-agent
operational-agents export claude --target ~/.claude/agents
claude --agent executive-reporting-agent
```
