...
> Detecta permisos excesivos, cuentas obsoletas, escaladas de privilegio y conflictos de segregación de funciones.
...
Identificar riesgos IAM verificables y conflictos SoD con evidencia de asignación, uso y vigencia.
...
Úsalo para revisar identidades y autorizaciones financieras sin cambiar cuentas, roles ni políticas.
...
### 1 · Caso financiero — Auditoría IAM normal

**El caso.** El inventario tiene 286 identidades, 34 roles y registros de uso de noventa días para entornos financieros. Los identificadores están completos y el período abarca veinticuatro horas de operación autorizada.

**Le escribes:**

```text
Audita permisos, cuentas inactivas y segregación de funciones en este inventario IAM.
```
...
- **Resultado estructurado** — `iam_findings` con referencias de entrada y evidencia.
...
### 2 · Caso financiero — Auditoría IAM con excepción

**El caso.** Una cuenta de operaciones heredó permisos de aprobación y ejecución después de un cambio de grupo. La discrepancia debe conservar su rastro hasta el registro de origen sin corregirlo.

**Le escribes:**

```text
Comprueba esta posible escalada de privilegio y el conflicto de segregación de funciones.
```
...
### 3 · Caso financiero — Auditoría IAM con falso positivo

**El caso.** Una cuenta break-glass parece inactiva, pero su excepción está documentada y revisada cada mes. El analista necesita saber por qué saltó la regla y qué evidencia permite cerrarla.

**Le escribes:**

```text
Determina si esta cuenta stale es un hallazgo real o una excepción de emergencia vigente.
```
...
flowchart LR
    ...
    AG(["🔐 iam-audit-agent"])
    OUT["📦 Te entrega<br/>· iam_findings<br/>· evidence_refs<br/>· confidence_assessment<br/>· human_review_packet<br/>· audit_events"]
...
```bash
operational-agents inspect iam-audit-agent
operational-agents export claude --target ~/.claude/agents
claude --agent iam-audit-agent
```
