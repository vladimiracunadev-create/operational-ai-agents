...
# ⛓️ Blockchain Monitoring Agent

> Verifica estado, confirmaciones, importes y contrapartes de transacciones on-chain mediante fuentes de solo lectura.
...
Entregar comprobaciones on-chain reproducibles, multifuente cuando sea posible, sin interactuar con contratos ni wallets.
...
Úsalo para comprobar una transacción o lote on-chain y separar estado observado de interpretación operacional.
...
### 1 · Caso financiero — Monitoreo on-chain normal

**El caso.** Un lote de 420 hashes debe verificarse contra dos endpoints de lectura y la política de confirmaciones de cada red. Los identificadores están completos y el período abarca veinticuatro horas de operación autorizada.

**Le escribes:**

```text
Comprueba el estado y las confirmaciones de este lote de transacciones on-chain.
```
...
- **Resultado estructurado** — `onchain_verification` con referencias de entrada y evidencia.
...
### 2 · Caso financiero — Monitoreo on-chain con excepción

**El caso.** El exchange marca un retiro como completado, pero el hash informado no aparece en el explorador autorizado. La discrepancia debe conservar su rastro hasta el registro de origen sin corregirlo.

**Le escribes:**

```text
Verifica este hash ausente y documenta qué puede y qué no puede concluirse.
```
...
### 3 · Caso financiero — Monitoreo on-chain con falso positivo

**El caso.** Una reorganización breve redujo confirmaciones y disparó una alerta que se resolvió en el bloque siguiente. El analista necesita saber por qué saltó la regla y qué evidencia permite cerrarla.

**Le escribes:**

```text
Determina si esta caída de confirmaciones sigue siendo un riesgo o fue una reorganización resuelta.
```
...
flowchart LR
    ...
    AG(["⛓️ blockchain-monitoring-agent"])
    OUT["📦 Te entrega<br/>· onchain_verification<br/>· evidence_refs<br/>· confidence_assessment<br/>· human_review_packet<br/>· audit_events"]
...
| Propiedad | Valor |
|---|---|
| Identificador | `blockchain-monitoring-agent` |
...
```bash
operational-agents inspect blockchain-monitoring-agent
operational-agents export claude --target ~/.claude/agents
claude --agent blockchain-monitoring-agent
```
